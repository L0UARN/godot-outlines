...
void main() {
	...
	if (distance_to_closest_seed >= 0.0f && distance_to_closest_seed < 1.0f) {
		imageStore(output_image, current_position, vec4(closest_seed.x, closest_seed.y, 1.0f, closest_seed.a));
	} else {
		imageStore(output_image, current_position, vec4(closest_seed.x, closest_seed.y, 0.0f, closest_seed.a));
	}
}
