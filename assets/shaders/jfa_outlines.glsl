...
layout(rgba8, set = 1, binding = 0) uniform restrict readonly image2D original_image;
...
void main() {
	...
	if (current_seed_position.x == 0.0f && current_seed_position.y == 0.0f) {
	...
	vec3 original_pixel = imageLoad(original_image, current_seed_position).xyz;
	...
	if (distance_to_seed > outlinesSize - 1.0f) {
		...
		imageStore(output_image, current_position, vec4(original_pixel, smoothing));
		return;
	}
	...
	imageStore(output_image, current_position, vec4(original_pixel, 1.0f));
}
